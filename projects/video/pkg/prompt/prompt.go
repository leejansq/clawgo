...
- script: 台词或旁白，口语化但专业
- style: 风格/质感（如：写实、赛博朋克、水墨、像素风、电影感、日系等）
...
评分标准：
- 每个维度 1-10 分
- 总分 = (structure + script + visual + camera + pacing + effects) / 6
- 通过标准：总分 >= 7 且每个维度 >= 5
...
func BuildScriptwriterPrompt(input *schema.ScriptwriterInput) string {
	...
	// 构建基本要求
	requirements := fmt.Sprintf(`请围绕主题《%s》创作一个分镜头视频脚本：
...
2. 必须包含运镜方式和专业景别术语
3. 画面描述要具体，便于AI理解和生成
4. 输出必须是JSON格式的分镜头脚本`, input.Theme, input.Theme, input.TargetAudience, input.Duration, sceneCount)

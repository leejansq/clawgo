...
分镜头脚本格式（JSON）：
{
  ...
  "global_style": "【重要】统一视觉风格，如：电影感、赛博朋克、水墨风，写实摄影风格。所有资产prompt必须包含此风格关键词，确保视觉一致。",
  "assets": [
    {
      "name": "科学家李博士",
      "type": "character",
      "description": "40岁中年男性科学家，穿白大褂，戴眼镜，表情专业自信",
      "prompt": "中年男性科学家，白大褂，戴眼镜，专业自信表情，实验室环境，电影感，高清",
      "negative": "卡通，低质量，变形，水印，签名"
    },
    {
      "name": "AI机器人",
      "type": "character",
      "description": "未来科技风格的AI机器人，蓝色光效，金属质感",
      "prompt": "未来AI机器人，金属质感，蓝色LED光效，科幻风格，高清，8K",
      "negative": "卡通，低质量，变形，水印"
    }
  ],
  "scenes": [
    {
...
关键要求：
- 【最核心】主题约束：所有脚本内容必须严格围绕指定主题展开！
  - 每一句台词、每一个画面描述都必须与主题相关
  - 禁止生成与主题无关的内容（如：主题是AI大模型，但写了美食、旅游等不相关的内容）
  - 如果研究资料中有与主题无关的内容，必须过滤掉
- global_style: 【最重要】统一视觉风格描述。所有资产的prompt必须包含此风格关键词，确保整个视频视觉一致
- assets: 【重要】全局资产（人物/道具/背景等），在视频开头统一定义，供各镜头引用
- 【风格强制】所有资产的prompt结尾必须包含global_style关键词，确保风格统一
- 每个资产包含：name(名称), type(类型), description(描述), prompt(提示词), negative(负面提示词)
- 资产类型：character(人物), prop(道具), background(背景), other(其他)
- 每个镜头 duration: 5-15秒
- description: 景别 + 主体描述（例："中景 - 主持人"）
- camera_move: 运镜方式（推/拉/摇/移/跟/升降/悬空/旋转/固定）
- visual: 详细的画面描述，包含光线、角度、氛围等，必须遵循global_style
- script: 台词或旁白，口语化但专业
- style: 当前镜头的风格强调（必须与global_style一致）
- text_effect: 文字动画效果（如：无、淡入淡出、打字机、闪烁、弹出等）
- light_effect: 光效（如：自然光、逆光、柔光、光斑、光剑、霓虹等）
- negative_prompt: 负面描述，防崩神器（如：模糊、变形、低质量、水印签名等）
- assets: 该镜头引用的资产名称列表（如：["科学家李博士", "实验室背景"]）
...
const ResearcherSystemPrompt = `你是视频脚本生成的研究员智能体，负责围绕目标人群的喜好和主题进行深度调研。

你的职责：
1. 深入了解目标受众的喜好、痛点、关注点
2. 搜集主题相关的内容、数据、案例
3. 分析同类爆款视频的成功要素
4. 提取对目标受众有吸引力的关键信息
...
研究重点：
【目标人群分析】
- 目标受众是谁？他们的年龄、职业、兴趣特征
- 他们关心什么？痛点需求是什么
- 他们喜欢什么样的内容风格？幽默感、专业感、情感共鸣？
- 什么样的信息最能打动他们？
...
{
  "overview": "主题概述和目标人群画像（100字以内）",
  "audience_insights": ["目标人群的喜好1", "目标人群的痛点2", ...],
...
1. 【最重要】主题相关性（Theme）：
   - 每一句台词是否都与指定主题相关
   - 每一个画面描述是否都与主题相关
   - 是否有内容偏离主题（如：主题是AI，但写了美食、旅游等）
   - 如果发现任何与主题无关的内容，该维度直接判为低分
...
7. 时长（Duration）：
   - 所有镜头时长总和是否接近目标时长
   - strict_duration=true 时：必须严格匹配，总时长必须精确等于目标时长，误差0秒
   - strict_duration=false 时：参考目标时长，允许误差范围 ±15秒
   - 如果偏差过大，必须返回 approved: false 并给出调整建议

8. 效果（Effects）：
   - 风格（style）是否与主题匹配？是否有特色
   - 文字特效（text_effect）是否合适
   - 光效（light_effect）描述是否专业、有创意
   - 负面描述（negative_prompt）是否有效防止生成崩坏

9. 空间结构（Spatial）：
   - 场景中物体、人物的空间位置关系是否合理
   - 物体、人物的比例是否符合常识（如：人物不能比房子大）
   - 镜头角度与空间描述是否匹配（如：俯视镜头描述不应出现仰视效果）
   - 场景过渡时空间连续性是否一致（如：从室内切到室外不能还在同一房间）
   - 资产的引用是否合理（如：某镜头引用了尚未出现的资产）

评分标准：
- 每个维度 1-10 分
- **Theme 维度最关键**：如果 Theme < 5，其他维度分数再高也要返回 approved: false
- 总分 = (theme + structure + script + visual + camera + pacing + effects + spatial) / 8
- 通过标准：总分 >= 7 且每个维度 >= 5
...
**特别强调**：如果发现任何与指定主题无关的内容（如：主题是AI大模型，但写了美食、旅游等内容），theme 维度直接给 1-3 分，并返回 approved: false。
...
func BuildScriptwriterPrompt(input *schema.ScriptwriterInput) string {
	...
	if input.HumanFeedback != "" {
		...
		humanFeedbackSection += "请结合人工修改意见，对脚本进行调整优化。\n"
	}
	...
	// 构建基本要求
	requirements := fmt.Sprintf(`【【【最高优先级】】】你需要创作与主题《%s》相关的内容！
...
2. 必须包含运镜方式和专业景别术语
3. 画面描述要具体，便于AI理解和生成
4. 输出必须是JSON格式的分镜头脚本`, input.Theme, input.Theme, input.Theme, input.TargetAudience, input.Duration, sceneCount)

	return requirements + researchSection + iterationSection + humanFeedbackSection
}
...
3. 画面描述是否清晰具体
4. 整体节奏是否有起伏
5. **总时长是否接近目标时长（允许%s误差）**
6. **如果总时长偏差过大，必须给出调整建议**
`, input.Theme, input.Script, input.Duration, strictText, tolerance, tolerance)
}
